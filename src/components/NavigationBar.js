    ...
    <React.Fragment>
      ...
      <Box sx={{ display: { xs: 'none', md: 'flex' } }}>
        {props.isLoggedIn ? <Button  color="inherit" className='logout' onClick={() => {
          ...
          navigate('/');
        }}>Logout</Button> : <Button style={{backgroundColor:"#f50057"}} color="inherit"><Anchor to='login'>Login</Anchor></Button>}
      </Box>
    </React.Fragment>
  )
}
      ...
      <Box display='fixed' className='nav-bar' sx={{ flexGrow: 1}}>
        <AppBar position="static">
          <Toolbar>
            ...
            <Box sx={{ flexGrow: 1 }} />
            <Link style={{marginRight:'10px',marginTop:'2px'}} component="button" color="inherit" variant="body2" onClick={() => {navigate('/')}}>Home</Link>
